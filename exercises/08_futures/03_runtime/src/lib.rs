    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let jh1 = tokio::spawn(reply_helper(first, Arc::clone(&reply)));
    let jh2 = tokio::spawn(reply_helper(second, reply));

    tokio::join!(jh1, jh2);
}

pub async fn reply_helper<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
