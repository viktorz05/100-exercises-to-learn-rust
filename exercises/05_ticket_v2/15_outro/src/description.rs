...
#[error("{invalid_descr}")]
pub struct TicketDescriptionError {
    invalid_descr: String
}
...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {
            Err(TicketDescriptionError { invalid_descr: String::from("The description cannot be empty") })
        } else if value.len() > 500 {
            Err(TicketDescriptionError { invalid_descr: String::from("The description cannot be longer than 500 bytes") })
        } else {
